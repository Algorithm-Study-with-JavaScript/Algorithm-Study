...
143
*/
const mySolution = (n, k, arr) => {
  const sum = new Set();

  // 3장을 뽑아서 합한 값들을 모두 Set에 저장
  for (let i = 0; i < n; i++) {
  ...
  // Set을 배열로 변환하고 내림차순으로 정렬
  const sortedSum = Array.from(sum).sort((a, b) => b - a);

  // K번째로 큰 값을 찾아 반환
  return sortedSum.length < k ? -1 : sortedSum[k - 1];
};

console.log(mySolution(10, 3, [13, 15, 34, 23, 45, 65, 33, 11, 26, 42]));
